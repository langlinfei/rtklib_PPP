...
static int ddidx(rtk_t *rtk, int *ix)
{
    ...
    for (m=0;m<6;m++) { /* m=0:GPS/SBS,1:GLO,2:GAL,3:BDS,4:QZS,5:IRN */
        ...
        for (f=0,k=na;f<nf;f++,k+=MAXSAT) {  /* k=number of fixed states ; f = number of frequencies; i = satellite */
...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa)
{
    ...
    trace(5, "Lambda DP:\n");  tracemat(5, DP,  nb, nx-na, 13, 4);
    trace(5, "Lambda Qb:\n");  tracemat(5, Qb,  nb, nb, 13, 4);
    trace(5, "Lambda Qab:\n"); tracemat(5, Qab, na, nb, 13, 4);

    trace(5, "Lambda y:\n");   tracemat(5, y, 1, nb, 13, 4);
                  ...
                  const nav_t *nav)
{
    ...
    trace(5, "update state rtk->x:\n"); tracemat(5, rtk->x,rtk->nx,1,13,4);
    trace(5, "update state rtk->P:\n"); tracemat(5, rtk->P, rtk->nx, rtk->nx, 13, 4);
    ...
    if (stat!=SOLQ_NONE&&resamb_LAMBDA(rtk,bias,xa)>1) {
        trace(5, "Before resamb_LAMBDA rtk->x:\n");   tracemat(5, rtk->x, 1, rtk->nx, 13, 4);
        trace(5, "After resamb_LAMBDA xa:\n");   tracemat(5, xa, 1, rtk->nx, 13, 4);
